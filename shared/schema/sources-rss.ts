...
export const rssItems = pgTable("rss_items", {
  ...
  // Favorites (News Hub feature)
  isFavorite: boolean("is_favorite").default(false).notNull(), // Is article in favorites
  favoritedAt: timestamp("favorited_at"), // When added to favorites
  userNotes: text("user_notes"), // User notes/remarks about the article
  ...
  // Article potential analysis (Level 2 - detailed analysis)
  articleAnalysis: jsonb("article_analysis"), // Full ArticlePotentialResult from analyzeArticlePotential()

  // Article translation cache
  articleTranslation: jsonb("article_translation"), // { text: string, language: 'ru', timestamp: string }
  ...
  index("rss_items_is_favorite_idx").on(table.isFavorite),
]);
