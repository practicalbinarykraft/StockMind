 ...
 * Used when we need to access the plain text key (e.g., for API calls)
 * encryptedKey remains encrypted, decryptedKey contains the plain text
 */
export type ApiKeyWithDecrypted = Omit<ApiKey, 'encryptedKey'> & {
  encryptedKey: string; // remains encrypted (original value from DB)
  decryptedKey: string; // decrypted plain text key
};
