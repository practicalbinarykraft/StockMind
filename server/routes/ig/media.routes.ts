...
router.get('/:accountId/media', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[Instagram Media] Fetching media for account: ${account.igUsername}`);
    ...
    console.log(`[Instagram Media] Saving ${mediaResponse.data.length} media items to database`);
...
router.get('/media/:igMediaId/details', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.error('[Instagram Media] Error fetching media details:', error);
    res.status(500).json({
