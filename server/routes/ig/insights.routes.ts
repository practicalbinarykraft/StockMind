...
router.get('/media/:igMediaId/insights', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[Instagram Insights] Error fetching insights:', { error });
    res.status(500).json({
...
router.post('/media/:igMediaId/sync', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`[Instagram Insights] Syncing insights for media: ${media.igMediaId}`);
    ...
    try {
      ...
      logger.info(`[Instagram Insights] Collected metrics:`, { metrics: Object.keys(metrics) });
      ...
      // Если ответ уже отправлен, просто логируем ошибку
      throw syncError;
    }
  } catch (error) {
    logger.error('[Instagram Sync] Error syncing media:', { error });
