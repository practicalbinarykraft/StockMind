...
export function registerRssRoutes(app: Express) {
  ...
  // POST /api/settings/rss-sources - Create new RSS source //antropic
  ...
  // POST /api/settings/rss-sources/:id/parse - Manually trigger RSS parsing //antropic
  app.post(
    "/api/settings/rss-sources/:id/parse",
    requireAuth,
    async (req: any, res) => {
      try {
        const userId = getUserId(req);
