...
export function registerReanalysisRoutes(app: Express) {
   ...
   */
  app.post("/api/projects/:id/reanalyze/start", requireAuth, async (req: Request, res: Response) => {
    try {
...
• Репосты: ${metrics.predicted.shares}`;
   ...
   * Check job status
   */
  app.get("/api/projects/:id/reanalyze/status", requireAuth, async (req: Request, res: Response) => {
    try {
      const userId = getUserId(req);
      if (!userId) return apiResponse.unauthorized(res);
