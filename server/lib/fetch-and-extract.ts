...
/**
 * Clean article title - remove extra whitespace and normalize
 */
...
/**
 * Clean article content - remove extra whitespace, dates, and junk
 */
function cleanArticleContent(text: string): string {
  ...
  // Remove common date patterns (e.g., "January 15, 2024", "15.01.2024", "2024-01-15")
  ...
  // Remove lines that are too short (likely junk)
  cleaned = cleaned.split('\n')
    .filter(line => {
      const trimmed = line.trim();
      // Keep lines that are meaningful (more than 20 chars or contain sentence-ending punctuation)
...
export async function fetchAndExtract(url: string): Promise<{
  ...
  try {
    ...
    for (let hop = 0; hop < 5; hop++) {
      ...
      const cleanedContent = cleanArticleContent(article.textContent);
      const cleanedTitle = article.title ? cleanTitle(article.title) : undefined;
