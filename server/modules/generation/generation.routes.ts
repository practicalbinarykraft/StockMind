...
router.post("/start", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[Generation] Запуск batch генерации для ${targetNewsIds.length} новостей:`,
      targetNewsIds,
    );
    ...
    const customPrompts =
      (conveyorSettings.customPrompts as {
        writerPrompt?: string;
        editorPrompt?: string;
      }) || {};
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const scriptExamples =
        (conveyorSettings.scriptExamples as string[] | null) || [];
