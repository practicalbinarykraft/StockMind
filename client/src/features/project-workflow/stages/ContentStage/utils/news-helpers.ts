...
export function filterNews(
  ...
  return items.filter((item) => {
    ...
    const matchesDismissed =
      !filters.hideDismissed || item.userAction !== "dismissed";

    const matchesUsed = !filters.hideUsed || item.userAction !== "selected";

    const matchesFreshness =
      filters.freshnessFilter === "all" ||
      item.freshnessLabel === filters.freshnessFilter;

    const matchesScore = !item.aiScore || item.aiScore >= filters.minScore;
