...
import type { Voice, Stage4StepData, UseStage4DataReturn } from "../types"

export function useStage4Data(): UseStage4DataReturn {
  const { project, getStepData } = useStageData()
  const projectId = project.id
  const stepData = getStepData(3) // Data from Stage 3
