...
/**
 * ProjectWorkflow - главный компонент workflow создания видео
 * Управляет загрузкой данных проекта, навигацией по этапам и обработкой ошибок
 */
export function ProjectWorkflow() {
  ...
  const { setProject, setSteps, reset } = useWorkflowStore();
  ...
  // Sync project data with workflow store
  useEffect(() => {
    if (project) {
      console.log("[ProjectWorkflow] Project loaded:", {
        id: project.id,
  ...
  // Sync steps data with workflow store
  useEffect(() => {
    if (steps) {
      setSteps(steps);
    }
