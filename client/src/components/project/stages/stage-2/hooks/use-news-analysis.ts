...
export function useNewsAnalysis(filteredNews: EnrichedRssItem[]) {
  ...
  const [translations, setTranslations] = useState<Record<string, { text: string; language: 'en' | 'ru' }>>({})
  ...
  useEffect(() => {
    ...
    const loadedTranslations: Record<string, { text: string; language: 'en' | 'ru' }> = {}
    ...
    filteredNews.forEach(item => {
      ...
      if (articleTranslation && articleTranslation.text && articleTranslation.language === 'ru') {
    ...
    if (foundAnalysisCount > 0 || foundTranslationCount > 0) {
      console.log(`[useNewsAnalysis] Loaded ${foundAnalysisCount} analyses and ${foundTranslationCount} translations from ${filteredNews.length} articles`)
    }
    ...
    if (Object.keys(loadedTranslations).length > 0) {
      setTranslations(prev => {
        // Check if we actually have new data
        const hasNewData = Object.keys(loadedTranslations).some(id => !prev[id])
        if (!hasNewData) return prev
  ...
  useEffect(() => {
    const articlesToTranslate = filteredNews.filter(item => {
      ...
      if (dbTranslation && dbTranslation.text && dbTranslation.language === 'ru') {
    ...
    if (articlesToTranslate.length > 0) {
      console.log(`[useNewsAnalysis] Auto-translating ${articlesToTranslate.length} articles (${filteredNews.length - articlesToTranslate.length} already have translations)`)
        ...
        setTimeout(() => {
          batch.forEach(item => {
            ...
            translateMutation.mutate({ 
              ...
              onSuccess: () => {
                console.log(`[useNewsAnalysis] ✅ Auto-translated article ${item.id}`)
              },
              onError: (error) => {
                console.warn(`[useNewsAnalysis] ⚠️ Failed to auto-translate article ${item.id}:`, error)
              }
            })
          })
  ...
  const translateMutation = useMutation({
    mutationFn: async ({ itemId, text, showToast = false }: { itemId: string; text: string; showToast?: boolean }) => {
      const res = await apiRequest("POST", "/api/news/translate", { text, articleId: itemId })
  ...
  const analyzeMutation = useMutation({
    mutationFn: async ({ itemId, title, content }: { itemId: string; title: string; content: string }) => {
      console.log(`[useNewsAnalysis] Starting analysis for article ${itemId}`, {
        title: title.substring(0, 50),
        contentLength: content.length,
      })
      const res = await apiRequest("POST", "/api/news/analyze", { title, content, articleId: itemId })
      const response = await res.json()
    ...
    onSuccess: (data: any, variables) => {
      console.log(`[useNewsAnalysis] onSuccess called for article ${variables.itemId}`, {
        rawData: data,
        hasData: !!data.data,
      ...
      if (!analysisData || (!analysisData.score && !analysisData.overallScore)) {
        console.error(`[useNewsAnalysis] Invalid analysis data for article ${variables.itemId}`, analysisData)
        toast({
      ...
      console.log(`[useNewsAnalysis] Analysis completed for article ${variables.itemId}`, {
  ...
  const analyzeBatchMutation = useMutation({
    mutationFn: async (articles: Array<{ id: string; title: string; content: string }>) => {
      const res = await apiRequest("POST", "/api/news/analyze-batch", { articles })
  ...
  const handleAnalyze = async (item: EnrichedRssItem) => {
    ...
    if (!item.fullContent) {
      try {
        const fullContentResponse = await apiRequest("POST", `/api/news/${item.id}/fetch-full-content`, {})
  ...
  const loadSavedAnalysisMutation = useMutation({
    mutationFn: async (itemId: string) => {
      ...
      try {
        ...
        console.log(`[useNewsAnalysis] Response status: ${res.status}, content-type: ${contentType}`)
        
        if (!contentType.includes('application/json')) {
          const text = await res.text()
          console.error(`[useNewsAnalysis] ❌ Server returned non-JSON response:`, {
            contentType,
            preview: text.substring(0, 200),
            isHTML: text.includes('<!DOCTYPE'),
          })
          throw new Error('Сервер вернул некорректный ответ. Возможно, endpoint не зарегистрирован.')
        }
        ...
        console.error(`[useNewsAnalysis] Error loading analysis for article ${itemId}:`, error)
        // Re-throw to trigger onError
        throw error
      }
    },
    onSuccess: (analysisData: any, itemId: string) => {
      console.log(`[useNewsAnalysis] ✅ Loaded saved analysis for article ${itemId}`, {
        score: analysisData?.score,
        verdict: analysisData?.verdict,
      })
    ...
    onError: (error: any, itemId: string) => {
      console.error(`[useNewsAnalysis] ❌ Failed to load analysis for article ${itemId}:`, error)
