...
export function useNewsMutations(project: Project) {
  ...
  const parseExtendedMutation = useMutation({
    mutationFn: async (params: { startDate?: string; endDate?: string }) => {
      const res = await apiRequest("POST", "/api/news/refresh-extended", params)
      return await res.json()
    },
    onSuccess: (data: any, variables) => {
      queryClient.invalidateQueries({ queryKey: ["/api/news"] })
      const message = variables.startDate && variables.endDate 
        ? `Parsed ${data.totalProcessed || 0} items, ${data.newItems || 0} new (RSS feeds may not support full date range)`
  ...
  const proceedMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.refetchQueries({ queryKey: ["/api/projects", project.id, "steps"] })
