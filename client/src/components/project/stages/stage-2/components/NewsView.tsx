      ...
      <Card>
        <CardContent className="py-16 text-center">
          <p className="text-muted-foreground">
            No news articles found. {allNews && allNews.length > 0 ? 'Try adjusting filters.' : 'Add RSS sources in Settings.'}
          </p>
      ...
      {groupedNews.hot.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-semibold">Hot News ({groupedNews.hot.length})</h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {groupedNews.trending.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-semibold">Trending ({groupedNews.trending.length})</h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {groupedNews.recent.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-semibold">Recent ({groupedNews.recent.length})</h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {groupedNews.old.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-semibold">Older ({groupedNews.old.length})</h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {(showAllOldNews ? groupedNews.old : groupedNews.old.slice(0, 6)).map(item => (
              <NewsCard
                key={item.id}
                item={item}
                onSelect={onSelect}
