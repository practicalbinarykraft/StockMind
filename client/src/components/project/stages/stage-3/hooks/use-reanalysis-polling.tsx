...
export function useReanalysisPolling(
  ...
  lastSubmittedPayload: MutableRefObject<{
    scenes: any[];
    fullScript: string;
  } | null>
) {
  ...
  useEffect(() => {
    ...
    const savedPayloadProjectId = localStorage.getItem(
      "reanalyzePayloadProjectId"
    );

    console.log(
      `reanalyze jobId: ${savedJobId}/// projectId: ${savedProjectId}/// payload: ${savedPayload}/// payloadProject: ${savedPayloadProjectId}`
    );
      ...
      const interval = setInterval(async () => {
        try {
          const res = await fetch(
            `/api/projects/${projectId}/reanalyze/status?jobId=${savedJobId}`
          );
