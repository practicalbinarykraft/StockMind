...
export function Stage3AIAnalysis({ project, stepData, step3Data }: Stage3Props) {
  ...
  const hasGeneratedVariants = !!(step3DataContent?.generatedVariants || step3DataContent?.step === "constructor")
  ...
  const [currentStepState, setCurrentStepState] = useState<"load" | "constructor">(currentStep)
  ...
  const handleGenerateFromStep3_1 = async (data: { sourceContent: string; format: string; customPrompt?: string }) => {
  ...
  const useNewFlow = !STAGE3_MAGIC_UI || (currentStepState === "load" && !generatedData)
