...
export function Stage2ContentInput({ project, stepData }: Stage2Props) {
  ...
  const { refreshMutation: refreshRssMutation, proceedMutation } = useNewsMutations(project)
  ...
  const handleInstagramContinue = async () => {
    ...
    const res = await apiRequest("GET", `/api/instagram/items/${instagramItemId}`)
  ...
  const handleCreateScript = async (item: EnrichedRssItem, analysis: any) => {
    try {
      ...
      const recommendedFormat = analysis?.breakdown?.recommendedFormat?.format || 'news_update'
              ...
              {enrichedFilteredArticles.map((article) => {
                ...
                return (
                  <NewsListItem
                    ...
                    onShowSavedAnalysis={(itemId) => loadSavedAnalysisMutation.mutate(itemId)}
                  />
                )
              })}
                </div>
