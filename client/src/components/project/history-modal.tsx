...
import { History, RotateCcw, CheckCircle2, User, Sparkles } from 'lucide-react';
...
export function HistoryModal({ projectId, currentScenes, onClose, onRevert }: HistoryModalProps) {
  ...
  const getChangeLabel = (version: ScriptVersion) => {
    ...
    if (type === 'apply_recommendation') return 'Применена рекомендация';
    if (type === 'apply_all') return 'Применены все рекомендации';
