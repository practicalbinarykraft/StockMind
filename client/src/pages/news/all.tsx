...
export default function NewsAll() {
  ...
  const { data: articles, isLoading } = useQuery<RssItem[]>({
    ...
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/news/all?source=${sourceFilter}&score=${scoreFilter}&sort=${sortBy}`)
  ...
  const { refreshRssMutation, createProjectMutation, toggleFavoriteMutation } = useNewsMutations()

  const handleCreateScript = async (item: EnrichedRssItem, analysis: any) => {
    // Generate script and save to library
    try {
      const format = analysis?.breakdown?.recommendedFormat?.format || 'news_update'
      const res = await apiRequest("POST", `/api/articles/${item.id}/generate-script`, {
  ...
  const { data: sources } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log(`[NewsAll] Loaded ${sourcesList.length} RSS sources for filter`)
      return sourcesList
    },
  })
  ...
  const filteredArticles = useMemo(() => {
    ...
    if (viewMode === "favorites") {
      baseArticles = baseArticles.filter((article) => article.isFavorite === true)
    }
    
    // Apply source filter
    if (sourceFilter !== "all") {
      baseArticles = baseArticles.filter((article) => article.sourceId === sourceFilter)
    }
    
    // Apply score filter
    if (scoreFilter !== "all") {
      if (scoreFilter === "high") {
        baseArticles = baseArticles.filter((article) => (article.aiScore ?? 0) >= 80)
        ...
        baseArticles = baseArticles.filter((article) => (article.aiScore ?? 0) < 50)
      }
    }
    ...
    return filtered
  }, [articles, searchTerm, viewMode, statusFilter, sourceFilter, scoreFilter, sortBy])
          ...
          <Card className="mb-6">
            <CardContent className="pt-6">
              ...
              <div className="flex gap-2 mb-4 pb-4 border-b">
                <Button
                  variant={viewMode === "all" && statusFilter === "all" ? "default" : "outline"}
                  onClick={() => {
                    setViewMode("all")
                    setStatusFilter("all")
                  }}
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <Button
                  ...
                  className="gap-2"
                >
                  <RefreshCw className={`h-4 w-4 ${refreshRssMutation.isPending ? 'animate-spin' : ''}`} />
                  Обновить новости
                </Button>
              </div>
            </CardContent>
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {enrichedFilteredArticles.map((article) => {
                ...
                const isLoadingSavedAnalysis = loadSavedAnalysisMutation.isPending && loadSavedAnalysisMutation.variables === article.id
                ...
                return (
                  <NewsListItem
                    ...
                    onTranslate={(itemId, text) => handleTranslate(itemId, text)}
                    onAnalyze={(item) => handleAnalyze(item)}
                    onLoadSavedAnalysis={handleLoadSavedAnalysis}
                    onCreateScript={handleCreateScript}
                    onToggleFavorite={(isFavorite) => {
                      toggleFavoriteMutation.mutate({ id: article.id, isFavorite })
                    }}
                  />
                )
              })}
